<script>
  ...
  const searchParams = new URLSearchParams(
    Object.assign({}, selection, { lok: $cookiesAllowed })
  ).toString();
    ...
    .then(res => {
      return res.filter(
        cluster =>
          ...
          !(cluster.name === "Zuschuss" && selection.time == "6 Monate")
      );
    });

  let selectedTab =
    (location.hash && decodeURI(location.hash.substring(1))) || "Zuschuss";
...
<div class="container-fluid">
  <div class="d-flex flex-column align-items-center">
    <h3 class="mt-4 mb-4">Deine Resultate</h3>
    <div class="info-text sharing">
      <p class="text-center">
        Speicher den Link zu deinem persönlichen Ergebnis:
      </p>
    ...
    <ul class="nav nav-pills sticky-top">
      ...
      {:then data}
        {#each data as cluster}
          <li class="nav-item">
            <a
              class="nav-link"
              on:click={() => (selectedTab = cluster.name)}
      ...
      <li class="nav-item">
        <a
          class="nav-link"
          on:click={() => (selectedTab = STEUERSTUNDUNG)}
          class:active={selectedTab === STEUERSTUNDUNG}
          href={'#' + STEUERSTUNDUNG}>
          {STEUERSTUNDUNG}
        </a>
      </li>
      {#if selection.employees !== 0}
        <li class="nav-item">
          <a
            class="nav-link"
            on:click={() => (selectedTab = KURZARBEIT)}
            class:active={selectedTab === KURZARBEIT}
            href={'#' + KURZARBEIT}>
            {KURZARBEIT}
          </a>
        </li>
        <li class="nav-item">
          <a
            class="nav-link"
            on:click={() => (selectedTab = SOZIALBEITRAEGE)}
            class:active={selectedTab === SOZIALBEITRAEGE}
            href={'#' + SOZIALBEITRAEGE}>
            {SOZIALBEITRAEGE}
          </a>
        </li>
      {/if}
      <li class="nav-item">
        <a
          class="nav-link"
          on:click={() => (selectedTab = ZIVILRECHT)}
          class:active={selectedTab === ZIVILRECHT}
          href={'#' + ZIVILRECHT}>
          {ZIVILRECHT}
        </a>
      </li>
  ...
  <div id="tab-content" class="mb-3">
          ...
          {#if help[cluster.name]}
            <div class="info-text">
              {#if $descriptions.find(d => d.name === cluster.name)}
                {@html $descriptions.find(d => d.name === cluster.name).html}
                <br />
              {/if}
              <a
                target="_blank"
                href={help[cluster.name] ? help[cluster.name].link : 'https://wir-bleiben-liqui.de'}>
                Du willst mehr wissen? Direkt zu unserem Blog
              </a>
            </div>
          {/if}
        {/if}
    ...
    <div class="info-text">
      {#if selectedTab === STEUERSTUNDUNG}
        {@html $descriptions
          .find(d => d.name === STEUERSTUNDUNG)
          .html.replace(
            '&lt;&lt;state&gt;&gt;',
            selection.state
              ? $bundeslaender.find(land => land.id == selection.state).name ||
                  ''
              : ''
          )}
        <br />
        <div class="row">
          {#if selection.state}
            <a
              target="_blank"
              class="info-link col-12 col-lg-4"
              href={steuerstundungen[selection.state]}>
              Antragsformular
            </a>
            <a
              target="_blank"
              class="info-link col-12 col-lg-4"
              href={finanzaemter[selection.state]}>
              Finanzamt
            </a>
          {/if}
          <a
            target="_blank"
            class="info-link col-sm-12 col-lg-4"
            href={weitereInfos.source}>
      ...
      {#if selectedTab === KURZARBEIT}
        ...
        <a target="_blank" href={help['Kurzarbeit'].link}>
          Du willst mehr wissen? Weitere Informationen
        </a>
        <br />
        <a target="_blank" href="https://kurzarbeit-einfach.de">
          Du willst einen Antrag stellen? Jetzt zu U:DO und Unterstützung
          erhalten!
        </a>
      {/if}
      {#if selectedTab === SOZIALBEITRAEGE}
        {@html $descriptions.find(d => d.name === SOZIALBEITRAEGE).html}
        <a target="_blank" href={help['Sozialbeiträge'].link}>
          Du willst mehr wissen? Weitere Informationen
        </a>
      {/if}
